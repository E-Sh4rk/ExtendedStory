...
let precedence_to_edge trace options (src,dest) =
...
let print_json_of_extended_story (fact,exps) eoi compression_algorithm options oc =
  ...
  let main_core = compression_algorithm (get_trace_explorer fact) (get_var_infos fact) (eoi::(IntSet.elements blocked_events)) in
  ...
  (* TODO : Add activations if verbose *)
